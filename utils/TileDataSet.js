...
export function pasteDataSet(dest, source, x = 0, y = 0) {
    const lrX = Math.min(x + source.width, dest.width)
    const lrY = Math.min(y + source.height, dest.height)
    for (let i = y; i < lrY; i++) {
        const y2 = i - y
        for (let j = x; j < lrX; j++) {
            const x2 = j - x
            const dat = source.getXY(x2, y2)
            dest.setXY(j, i, dat)
        }
    }
    return dest
    ...
    stitchTiles(tiles) {
        ...
        for (const im of tiles) {
            ...
            pasteDataSet(
                stitched,
                ds,
                md.imgX * this.tileWidth,
                md.imgY * this.tileHeight
            )
        }
